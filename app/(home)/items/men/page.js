...
import { getItemsByMenAction, getSectionsByGenderAction } from "@/actions/userItems-action";
...
export default function MenProductsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const itemsResponse = await getItemsByMenAction(currentPage);
      const sectionsResponse = await getSectionsByGenderAction('Men');
      ...
      <div className="relative w-full h-[300px] mx-auto overflow-hidden parent-container">
        <div className="w-full bg-watch h-[300px] bg-cover bg-center text-white flex justify-center items-center">
          <div className="flex justify-between max-w-[1000px] w-[80%] mt-8">
            <h1 className="sm:text-4xl text-2xl font-bold">Men</h1>
            <p className="text-xl">Home &rsaquo; Products &rsaquo; <span className="text-[#ef233c]">Men</span></p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-whitesmoke">
        <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
          <div className="flex justify-between">
            <h2 className="sm:text-4xl text-2xl font-bold tracking-tight text-gray-900">Men Section</h2>
            <DropDown sections={sections || []} href="men" />
            </div>
